...
public class SettleAllSchedulerJob extends QuartzJobBean {
	...
	public void executeInternal(JobExecutionContext jeContext) throws JobExecutionException {
		try {
			log.info("SettleAllSchedulerJob Job Started...");
			String targetUrl = environment.getProperty("SettleAllSchedulerJob.target.client");
			var response = jerseyClientHelper.doGETRequest(client, targetUrl);
			if (response.getStatus() == 200) {
				log.info("Successful response...");
			} else {
				log.error("SettleAll Job Scheduler Bad Request due to response: {}", response.getStatusInfo().getReasonPhrase());
			}
			log.info("SettleAllScheduler Job Ended...");
		} catch (Exception e) {
			log.error("SettleAll Job Scheduler Exception: {}", Throwables.getStackTraceAsString(e));
		}
	}
